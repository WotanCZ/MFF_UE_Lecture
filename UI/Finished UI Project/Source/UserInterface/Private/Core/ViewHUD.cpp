...
bool AViewHUD::ShowUI(FName ViewName)
{
	...
	TObjectPtr<UViewWidget> view = GetViewByName(ViewName);
...
void AViewHUD::Refocus()
{
	...
	TObjectPtr<APlayerController> controller = GetOwningPlayerController();
	TObjectPtr<UEnhancedInputLocalPlayerSubsystem> subsystem = ULocalPlayer::GetSubsystem<UEnhancedInputLocalPlayerSubsystem>(controller->GetLocalPlayer());
	...
	{
		TObjectPtr<UViewWidget> viewValue = ViewPair.Value;
	...
	const TObjectPtr<UViewWidget> view = GetViewByName(viewToFocus);
...
void AViewHUD::BeginPlay()
{
	...
	TArray<TObjectPtr<UViewWidget>> widgets;
	...
	for (TObjectPtr<UViewWidget>& view : widgets)
	{
		if (IsValid(view))
		{
			view->Created();
...
TObjectPtr<UViewWidget> AViewHUD::GetViewByName(FName ViewName)
{
